...
public class TerrainDiffusionMc implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
                ...
                dispatcher.register(literal("td-coordinates").executes(TerrainDiffusionMc::executeCoordinates));
        });
    }
    ...
    private static int executeCoordinates(CommandContext<ServerCommandSource> ctx) {
        if (!WorldBlueprintManager.enabled()) {
            ctx.getSource().sendFeedback(() -> Text.literal("No Azgaar blueprint is active in this world."), false);
            return 0;
        }
        var pos = ctx.getSource().getPosition();
        var c = WorldBlueprintManager.coordinates(pos.x, pos.z);
        ctx.getSource().sendFeedback(() -> Text.literal(String.format(java.util.Locale.ROOT,
                "X %.0f, Z %.0f — latitude %.5f°, longitude %.5f° (%s)", pos.x, pos.z,
                c.latitude(), c.longitude(), c.authored() ? "authored map" : "exterior")), false);
        return 1;
    }
}
